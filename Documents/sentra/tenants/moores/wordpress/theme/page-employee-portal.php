<?php
require __DIR__ . '/inc/page-employee-portal.php';
return;
/**
...
<section class="section" id="employee-portal" data-employee-portal data-api-base="<?php echo esc_attr($api_base); ?>" data-tenant-id="<?php echo esc_attr($tenant_id); ?>" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-signage-manager-url="<?php echo esc_url($signage_manager_url); ?>" data-signage-player-base="<?php echo esc_attr($signage_player_base); ?>" data-signage-device-id="<?php echo esc_attr($signage_device_id); ?>" data-signage-device-token="<?php echo esc_attr($signage_device_token); ?>">
		...
		<div data-portal-content hidden style="margin-top: var(--sp-30);">
			...
			<div class="panel edge pad" style="margin-top: var(--sp-40);">
				...
				<div class="strip" style="margin-top: var(--sp-20);">
					...
					<div class="tile">
						...
						<div class="value" data-employee-metric="invoices">—</div>
					</div>
					<div class="tile">
			...
			<div class="cards" style="margin-top: var(--sp-40);">
				...
				<div class="card">
					...
					<h3>Invoicing</h3>
					<p>Create invoices, track balances, and update payment status.</p>
					<div class="mc-auth-actions">
						<a class="btn btn-primary" href="#" data-employee-action="invoices">Open Invoices</a>
						<a class="btn btn-outline" href="#" data-employee-action="new-invoice">Create Invoice</a>
					</div>
				</div>
				<div class="card">
			...
			<div class="mc-manager-modal" data-employee-modal hidden>
				...
				<div class="mc-manager-panel panel edge pad" data-employee-manager>
					...
					<div class="mc-manager-tabs" role="tablist">
						...
						<button type="button" class="mc-manager-tab" data-manager-tab="invoices">Invoices</button>
	...
	if (!packet || !packet.access_token) {
		debugLog('auth_packet', packet || 'missing - attempting staff verification');
		// No packet - try staff verification directly
		const payload = new URLSearchParams();
		payload.set('action', 'moores_sentra_verify');
		payload.set('nonce', config.nonce);
		// Don't set access_token - let backend check staff database
		...
		.then(res => res.json())
		.then(data => {
			debugLog('staff_verify_response', data);
			...
			if (data?.success && authPacket && staff) {
				...
				localStorage.setItem('sentra_auth', JSON.stringify(authPacket));
				showContent();
		...
		.catch((err) => {
			debugLog('staff_verify_error', err?.message || err);
			showDenied();
		});
		return;
	}
	...
	const wireEmployeePanel = (authPacket) => {
		...
		const renderList = (key, data) => {
			...
			items.slice(0, 6).forEach((item) => {
				...
				if (key === 'jobs') {
					...
					label = item?.number || item?.invoice_no || `Invoice #${item?.id ?? ''}`;
					const amount = item?.amount || item?.total || item?.total_amount || 0;
					const status = item?.status ? String(item.status).replace('_', ' ') : 'draft';
					value = `${status} • $${amount}`;
				} else if (key === 'gallery') {
		...
		const renderManager = (key, data, focusCreate = false) => {
			...
			if (key === 'invoices') {
				managerTitle.textContent = 'Invoices Control';
				managerSubtitle.textContent = 'Create and manage invoices tied to jobs and clients.';
				addActionButton('Add Invoice', () => openFormModal());
				addActionButton('Refresh Invoices', async () => renderManager('invoices', await fetchEndpoint('invoices', { nocache: true })), true);
				renderInvoices(items, focusCreate);
				return;
			}
			if (key === 'services') {
		...
		const renderInvoices = (items, focusCreate) => {
			setFormHeader('New Invoice', 'Create an invoice and tie it to a job.');
			managerForm.innerHTML = `
				<div class="mc-manager-form-card">
					<div class="mc-manager-form-head">
						<h4>New Invoice</h4>
						<p>Create an invoice with due date, status, and notes.</p>
					</div>
					<form data-manager-form="invoices">
						<div class="mc-manager-grid">
							<label class="mc-manager-field">
								<span>Job</span>
								<select name="job_id" data-invoice-job-select>
									<option value="">No job</option>
								</select>
							</label>
							<label class="mc-manager-field">
								<span>Invoice Number</span>
								<input type="text" name="number" placeholder="INV-1001">
							</label>
							<label class="mc-manager-field">
							...
							<label class="mc-manager-field mc-manager-field-wide">
								<span>Notes</span>
								<textarea name="notes" rows="2" placeholder="Line items, payment terms, internal notes"></textarea>
							</label>
						</div>
			...
			const form = managerForm.querySelector('form[data-manager-form="invoices"]');
			const jobSelect = form?.querySelector('[data-invoice-job-select]');
			if (jobSelect) populateJobSelect(jobSelect, '');
			...
			if (form) {
				form.addEventListener('submit', async (event) => {
					...
					await sendEndpoint('invoices', 'POST', payload);
					renderManager('invoices', await fetchEndpoint('invoices', { nocache: true }));
				});
			}

			const sorted = [...items].sort((a, b) => {
				const aa = new Date(a?.due_date || a?.created_at || 0).getTime() || 0;
				const bb = new Date(b?.due_date || b?.created_at || 0).getTime() || 0;
				return bb - aa;
			});
			...
			list.className = 'mc-manager-list';
			if (!sorted.length) {
				list.innerHTML = '<p class="lead">No invoices yet. Create your first invoice above.</p>';
				managerList.innerHTML = '';
				managerList.appendChild(list);
			...
			sorted.forEach((invoice) => {
				const id = invoice?.id ?? invoice?.invoice_id ?? invoice?.number;
				if (!id) return;
				const amount = parseFloat(invoice?.amount ?? invoice?.total ?? invoice?.total_amount ?? 0) || 0;
				const row = document.createElement('article');
				row.className = 'mc-manager-item';
				row.innerHTML = `
					<div class="mc-manager-item-head">
						<div>
							<div class="mc-manager-item-title">${escapeHtml(invoice?.number || `Invoice #${id}`)}</div>
							<div class="mc-manager-item-sub">Job: ${escapeHtml(invoice?.job_id || '—')} · Client: ${escapeHtml(invoice?.client_name || invoice?.customer_name || '—')}</div>
						</div>
						<div class="mc-chip">$${escapeHtml(amount.toFixed(2))}</div>
					</div>
					<div class="mc-manager-item-grid">
						<label class="mc-inline-field">
							<span>Status</span>
						...
						<label class="mc-inline-field">
							<span>Due Date</span>
							<input type="date" data-field="due_date" value="${escapeHtml(toDateInput(invoice?.due_date || ''))}">
						</label>
						<label class="mc-inline-field">
							<span>Amount</span>
							<input type="number" step="0.01" min="0" data-field="amount" value="${escapeHtml(String(amount || ''))}">
						</label>
						<label class="mc-inline-field mc-inline-field-wide">
							<span>Notes</span>
							<textarea rows="2" data-field="notes">${escapeHtml(invoice?.notes || '')}</textarea>
						</label>
					</div>
					<div class="mc-manager-item-actions">
						<button class="btn btn-primary" type="button" data-save>Save</button>
						<button class="btn btn-outline" type="button" data-mark-paid>Mark Paid</button>
						<button class="btn btn-outline" type="button" data-delete>Delete</button>
					</div>
				`;
				...
				row.querySelector('[data-save]').addEventListener('click', async () => {
					...
					payload.amount = payload.amount ? parseFloat(payload.amount) : 0;
					await sendEndpoint('invoices', 'PUT', payload, id);
					renderManager('invoices', await fetchEndpoint('invoices', { nocache: true }));
				});

				row.querySelector('[data-mark-paid]').addEventListener('click', async () => {
					const payload = {
						status: 'paid',
				...
				row.querySelector('[data-delete]').addEventListener('click', async () => {
					await sendEndpoint('invoices', 'DELETE', null, id);
					renderManager('invoices', await fetchEndpoint('invoices', { nocache: true }));
				});
		...
		const populateFolderSelect = async (select, selectedValue) => {
			...
			items.forEach((job) => {
				...
				const est = parseFloat(job.estimated_hours || 0);
				const act = parseFloat(job.actual_hours || 0);
				const progress = est > 0 ? Math.min(act / est, 1) : (job.status === 'done' ? 1 : 0);
				const progressPct = Math.round(progress * 100);
				const row = document.createElement('div');
				row.className = 'mc-job-panel';
				row.innerHTML = `
					<div class="mc-job-panel-header">
						...
						<div class="mc-chip mc-chip-${escapeHtml(status)}">${escapeHtml(status.replace('_', ' '))}</div>
					</div>
					<div class="mc-job-panel-progress">
						<div class="mc-job-progress-bar" style="width:${progressPct}%"></div>
					</div>
					<div class="mc-job-panel-meta">
						<span>${progressPct}% complete</span>
						<span>${escapeHtml(act || 0)}h / ${escapeHtml(est || 0)}h</span>
					</div>
					${descriptionShort ? `<div class="mc-job-panel-desc">${escapeHtml(descriptionShort)}</div>` : ''}
					<div class="mc-job-panel-tags">
						${services.length ? services.map((svc) => `<span class="mc-pill">${escapeHtml(svc)}</span>`).join('') : '<span class="mc-muted">No services tagged</span>'}
		...
		document.querySelectorAll('[data-employee-action="new-job"], [data-employee-action="new-invoice"], [data-employee-action="upload"], [data-employee-action="invite"], [data-employee-action="pricing"], [data-employee-action="staff-new"]').forEach((btn) => {
			btn.addEventListener('click', async (event) => {
				event.preventDefault();
				const action = btn.dataset.employeeAction;
				const map = { 'new-job': 'jobs', 'new-invoice': 'invoices', 'upload': 'gallery', 'invite': 'partners', 'pricing': 'services', 'staff-new': 'staff' };
				...
				try {
					const data = target === 'staff'
						? await fetchStaffList({ nocache: true })
						: await fetchEndpoint(target, { nocache: true });
