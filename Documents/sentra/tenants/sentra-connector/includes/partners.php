...
/**
 * Fetch partners from Sentra Core API
 */
function sentra_get_partners($limit = 6) {

    $limit = max(1, (int) $limit);

    $cfg = sentrasystems_config();
    $cache_key = 'sentra_partners_' . md5(($cfg['base'] ?? '') . '|' . $cfg['tenant_id'] . '|' . $limit);
    ...
    $path = 'api/tenants/' . $cfg['tenant_id'] . '/partners';
    ...
    } elseif (!empty($response['partners']) && is_array($response['partners'])) {
        $partners = $response['partners'];
    }

    set_transient($cache_key, $partners, $cfg['cache_ttl']);
    sentrasystems_cache_store_stale($cache_key, $partners);
