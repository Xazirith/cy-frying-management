...
class SentraService(ABC):
    ...
    def __init__(self, service_name: str, port: int):
        ...
        self.auth_url = os.getenv('AUTH_URL', 'http://sentra-auth:8082')
        ...
        # Verify service signature on startup
        if not self._verify_signature():
            print(f"❌ SERVICE SIGNATURE VERIFICATION FAILED!")
            print(f"   This service is not officially signed.")
            print(f"   Sign it with: sentra-dev sign-service {service_name}")
            if os.getenv('ENFORCE_SIGNING', 'true').lower() == 'true':
                print(f"   Exiting due to security policy.")
                exit(1)
            else:
                print(f"   ⚠️  WARNING: Running unsigned service (ENFORCE_SIGNING=false)")
    ...
    def _verify_signature(self) -> bool:
        ...
        try:
            # Get the service directory from the calling class (not base_service)
            calling_file = inspect.getfile(self.__class__)
            service_dir = os.path.dirname(os.path.abspath(calling_file))
            sig_file = os.path.join(service_dir, '.signature')
            
            # Check if signature file exists
            if not os.path.exists(sig_file):
                print(f"⚠️  No signature file found at {sig_file}")
                return False
            ...
            try:
                response = requests.post(
                    f"{self.auth_url}/api/auth/verify-service",
                    json={
                        "service": self.service_name,
                        "hash": current_hash,
                ...
                if response.status_code == 200:
                    ...
                    if data.get("verified"):
                        print(f"✅ Service signature verified")
                        print(f"   Signed by: {data.get('signed_by', 'unknown')}")
                        return True
                    else:
                        print(f"❌ Signature verification failed: {data.get('error')}")
                        return False
                else:
                ...
                print(f"⚠️  Could not verify with auth service: {e}")
                print(f"   (Service may not be running yet)")
                # Allow startup if auth service not available
                return True
